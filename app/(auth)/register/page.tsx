...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn("Dev mode fallback redirect:", err);
      router.push("/dashboard");
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen flex flex-col justify-center items-center px-4 py-12 bg-background">
      <div className="w-full max-w-sm flex flex-col gap-6 animate-fade-in">
        ...
        <div className="flex flex-col items-center text-center gap-2">
          ...
          <p className="text-xs text-foreground-muted">Untangle your tasks and conquer your days</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <User className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-foreground-muted" />
                <input
                  type="text"
                  value={name}
